...
const RentCarFilter = ({ vehicles, onFilterChange }) => {
  ...
  // Calculate dynamic ranges based on available vehicles
  const minPrice =
    vehicles.length > 0 ? Math.min(...vehicles.map((v) => v.feePerKm || 0)) : 0;
  const maxPrice =
    vehicles.length > 0
      ? Math.max(...vehicles.map((v) => v.feePerKm || 50))
      : 50;
  ...
  // Initialize filters based on available vehicles
  useEffect(() => {
  ...
  return (
    <div>
      <h1 className="text-black text-xl sm:text-2xl font-semibold text-left">
        Filters
      </h1>
