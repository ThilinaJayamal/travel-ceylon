...
export const registerTaxi = async (req, res) => {
  try {
    ...
    if (provider.serviceId) return res.status(400).json({ success: false, message: "Can't create multiple services using a single account" });

    const {
      driverName, nic, drivingId, nicImg, drivingIdImg, contact, website,
      profilePic, perKm, location, chasyNo, vehicleNo, province, vehicleType
    } = req.body;
    ...
    res.status(201).json({ success: true, message: "Taxi registered successfully", data: taxiUser });
  } catch (error) {
    console.error(error);
...
export const getTaxiProfile = async (req, res) => {
  try {
    ...
    if (!taxi) return res.status(404).json({ success: false, message: "Taxi profile not found" });
...
export const updateTaxi = async (req, res) => {
  try {
    ...
    const fields = ["driverName", "contact", "website", "profilePic", "location"];
    fields.forEach(f => { if (req.body[f] !== undefined) taxi[f] = req.body[f]; });

    await taxi.save();
    res.status(200).json({ success: true, message: "Taxi updated successfully", data: taxi });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, message: "Server error", error: error.message });
  }
...
export const getAvailableTaxis = async (req, res) => {
  try {
    ...
    if (!date || !vehicleType || !pickup)
      return res.status(400).json({ success: false, message: "Please provide date, vehicleType and pickup location" });
...
export const createTaxiBooking = async (req, res) => {
  try {
    ...
    if (distance === -1)
      return res.status(400).json({ success: false, message: "Please check your pickup & dropup location names" });
    ...
    if (existingBooking) return res.status(400).json({ success: false, message: "Taxi is already booked for this date" });
    ...
    res.status(201).json({ success: true, message: "Taxi booked successfully", data: newBooking });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, message: "Server error", error: error.message });
  }
...
export const changeBookingState = async (req, res) => {
  try {
    ...
    const provider = await serviceProviderModel.findById(req.user);
    if (!provider) return res.status(404).json({ success: false, message: "Service account not found" });
